...
do 
   ...
   docker run --privileged --network $net --name  $ip-host${c} -d   -v  /tmp/calico-code/:/tmp/  -w  /tmp/  docker:17.06-dind
   sleep 2 # wait 2 second for the dockerd daemon to start

   ## (2): Set the containers log into the 'debug' mode 
   docker cp /tmp/daemon_etcd.json  $ip-host${c}:/etc/docker/daemon.json 
